...
class SignalProp(torch.nn.Module):
    ...
    def forward(self, g, ts, nf, groundtruth=False):
        ...
        with g.local_scope():
            ...
            # input transition of pi_nodes is known?? for what meaning
            g.apply_nodes(self.node_skip_level_o, ts['pi_nodes'])

            def prop_net(nodes, groundtruth):
                # for specified nodes, generate mess and aggregate them
            ...
            def prop_cell(nodes, groundtruth):
                es = g.in_edges(nodes, etype='cell_out')  # returning in-edge of given nodes
