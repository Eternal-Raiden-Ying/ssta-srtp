...
parser.add_argument(
    '--test_iter', type=int,default=15799,
    help='If specified, test a saved model instead of training')
parser.add_argument(
    '--checkpoint', type=str,default="08_atcd_specul",
...
def train(model, args):
    ...
    for epoch in range(1000):
        ...
        for k in random.sample(keys, batch_size):
            (g, ts) = data_train[k]
...
if __name__ == '__main__':
    ...
    if args.test_iter:
        ...
        checkpoint = torch.load('./checkpoints/{}/{}.pth'.format(args.checkpoint, args.test_iter))
        breakpoint()
